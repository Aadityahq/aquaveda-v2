...
check(
  "1e. Issue payload accepts optional severity/category, and works without them",
  () => {
...
check(
  "2i. Coordinate tuple with wrong length is invalid (Zod tuple arity)",
  () => {
...
check("3b. Too-short ObjectId string fails", () => {
  assert.equal(
    objectIdString.safeParse("507f1f77bcf86cd79943901").success,
    false,
  );
});
...
check("3d. Non-hex ObjectId string fails", () => {
  assert.equal(
    objectIdString.safeParse("not-an-objectid-string!!").success,
    false,
  );
});
...
check(
  "4d. Review decision 'approved' WITH feedback also passes (feedback is optional, not forbidden)",
  () => {
...
check("5b. Review decision 'rejected' WITHOUT feedback fails", () => {
  ...
  assert.equal(result.success, false);
  const feedbackIssue = result.error.issues.find((i) =>
    i.path.includes("feedback"),
  );
...
check(
  "5d. Review decision 'rejected' with whitespace-only feedback fails",
  () => {
...
check(
  "6c. Comment with an unrecognized refType fails (e.g. 'KNOWLEDGE')",
  () => {
...
for (const status of ALL_ISSUE_STATUSES) {
  check(
    `8. changeIssueStatusSchema accepts targetStatus="${status}" (shape only, not legality)`,
    () => {
      const result = changeIssueStatusSchema.safeParse({
        targetStatus: status,
      });
      assert.equal(result.success, true);
    },
...
check(
  "8f. changeIssueStatusSchema rejects an unrecognized status string",
  () => {
    const result = changeIssueStatusSchema.safeParse({
      targetStatus: "closed",
    });
...
check(
  "8g. changeIssueStatusSchema accepting 'open' as a target is deliberate, not an oversight",
  () => {
...
// Report
// ---------------------------------------------------------------------
console.log("\nPhase C validation-layer verification\n" + "=".repeat(40));
